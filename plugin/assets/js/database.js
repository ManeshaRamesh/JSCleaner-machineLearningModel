  ...
  return new Promise((resolve) => {
    ...
    DBOpenRequest.onupgradeneeded = function (event) {
      ...
      db.onerror = function (event) {
        console.log(
          window.jscleaner.ErrorDATABASEERROR + "Failed to load to database."
        );
      };
  ...
  if (
    (requestDetails.url.search(".js") !== -1 &&
      requestDetails.url.search(
        "http://92.99.20.210:9000/JSCleaner/JSCleaner/JSLabel.py"
      ) === -1) ||
    ...
    if (blockScripts.includes(requestDetails.url)) {
      ...
      Obj = {
        ...
        label: labelledScript.get(requestDetails.url)
          ? labelledScript.get(requestDetails.url).label
          : undefined,
      };
      tempObj = {
        message: Obj,
...
function blockRequests(removelistener, object, action) {
  return new Promise((resolve, reject) => {
    ...
    if (!object) {
      ...
      if (action === "add") {
        //just add the name of the script (for add item)
    ...
    // return blockScripts;
    console.log(
      "Scripts that are being blocked",
      blockScripts,
      blockScripts.length
    );
