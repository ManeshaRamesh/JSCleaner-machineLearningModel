...
window.jscleaner.Constants.labels = [
  "Advertising",
  "Analytics",
  "Social",
  "Video",
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      ...
      if (msg.from === "popup" && msg.subject === "modeUpdate") {
        ...
        if (Database.URLExceptions.get(msg.content.url)) {
          Database.updateItem("urls", {
            urlsName: msg.content.url,
            default: 1,
            scripts: msg.content.scripts,
          });
          // .then(()=>{
      ...
      if (msg.from === "popup" && msg.subject === "urlUpdate") {
        ...
        if (!Database.URLExceptions.get(msg.content.url)) {
          Database.addItem(
            { urlsName: msg.content.url, default: 0, scripts: URLscripts },
            "urls"
          );
          sendResponse("added");
        } else {
      ...
      function (details) {
        return new Promise((resolve, reject) => {
          if (
            ...
            if (!found && !scripts.includes(details.url)) {
              //if is script is in database
